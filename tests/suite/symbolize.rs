...
#[test]
#[ignore = "test requires loaded test_hexdump module and more"]
fn symbolize_kernel_module_dwarf() {
    ...
    let (addr, name) = kallsyms
        ...
        .expect("no `cleanup_module` symbol found for [test_hexdump] in /proc/kallsyms");
    ...
    let code_info = sym.code_info.unwrap();
    assert_eq!(code_info.file, OsStr::new("test_hexdump.c"),);
    assert!(
